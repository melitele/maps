...
function preprocessDataEvent(event) {
  if (
    !(event.dataType === 'source' && event.tile && (event.tile.state === 'loaded' || event.tile.state === 'unloaded'))
  ) {
    return true;
  }
  if (!event.style?.sourceCaches[event.sourceId].used) {
    return true;
  }
  const {
    source: { attribution: sourceAttribution, metadata }
  } = event;
  ...
  const attribution = [];
  for (const match of sourceAttribution.matchAll(/href="([^"]+)/g)) {
    attribution.push(match[1]);
  }
  ...
  // track tile loading
  event.sourceHostname = new URL(attribution[0]).hostname;
  return true;
}
...
async function map(node, options) {
  ...
  const gestures = mapGestures(self._m, options);
  ...
  function move(options, animate = true) {
    if (options.zoom) {
      options = { ...options, zoom: options.zoom - 1 };
    }
    self._m[animate && self._m.isStyleLoaded() ? 'easeTo' : 'jumpTo'](options);
  }
  ...
  function state(key, value) {
    if (key === undefined) {
      return self._m.getGlobalState();
    }
    if (typeof key === 'object') {
      Object.entries(key).forEach(([key, value]) => state(key, value));
      return;
    }
