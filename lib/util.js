...
function closestPoint(res, p1, i, path) {
  ...
  if (!p && Math.abs(res.point[0] - p1[0]) < res.margin[0] && Math.abs(res.point[1] - p1[1]) < res.margin[1]) {
    p = p1;
  }
  if (p) {
...
function findPointOnPath(point, path, margin) {
  ...
  if (point && path && margin) {
    ...
    if (result.idx !== undefined) {
      ...
      result.pol = result.p !== path[result.idx] &&
        (result.idx === path.length - 1 || result.p !== path[result.idx + 1]);
      return result;
    }
  }
}
