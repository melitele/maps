...
function map(node, options) {
  ...
  function queryRenderedFeaturesByName(text, options = {}) {
    const regexp = q2regexp(text);

    if (options.layers === undefined && styles) {
      options.layers = styles.poi;
    }

    options.filter = ({ properties: name }) => regexp.test(name);
    let features = self._m.queryRenderedFeatures(options);
    if (!features) {
      return [];
    }
