...
var makeFeatureEventHandler = require('./feature-event-handler');
...
function map(node, options) {
  ...
  function initStyles() {
    if (!styles) {
      var style = self._m.getStyle();
      if (style.layers) {
        styles = style.layers.reduce((result, layer) => {
          if (layer.metadata) {
            window.console.log(layer.id, layer.metadata);
  ...
  function units(u) {
    ...
    options.units = u;
    var units = mapUnits(options.units);
    applyUnits(units);
