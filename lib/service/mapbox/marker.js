...
function marker(options) {
  ...
  function image(icon, off, images) {
    ...
    if (self._under) {
      self._l.paint['circle-radius'] = Math.max(icon.size[0], icon.size[1]) / 2;
      self._l.paint['circle-translate'] = off && [0, - icon.size[1] / 2];
    }
