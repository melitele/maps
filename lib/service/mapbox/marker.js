...
function create(self, id, options) {
  var label = options.label, haloColor;
  ...
  if (label) {
    ...
    self._l.paint['text-color'] = '#FFFFFF';
    if (options.textBackground) {
      self._l.layout['text-size'] = 12;
      self._l.paint['text-translate'] = [0, SIZE[1] / 2 + 2];
      self._l.paint['text-halo-width'] = 1;
      haloColor = options.fillColor;
      haloColor = colors[haloColor] || haloColor;
      self._l.paint['text-halo-color'] = hex2rgba(haloColor);
    }
    else {
      self._l.layout['text-size'] = 10;
      self._l.paint['text-halo-width'] = 0.5;
