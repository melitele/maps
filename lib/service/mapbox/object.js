  ...
  function on(event, fn) {
    ...
    if (!drag[event]) {
      if (self._l) {
        ...
        if (self._m) {
          self._featureEventHandler.on(event, self._l.id, handler);
        }
      }
      else if (self._m) {
      ...
      if (listeners[event] && listeners[event].some(function (listener, i, listeners) {
        if (listener.fn === fn) {
          if (!drag[event]) {
            if (listener.layer) {
              if (self._m) {
                self._featureEventHandler.off(event, listener.layer, listener.handler);
              }
            }
            else {
    ...
    if (!self._m) {
      ...
      Object.keys(listeners).forEach(function(event) {
        if (!drag[event]) {
          listeners[event].forEach(function (listener) {
            if (listener.layer) {
              self._featureEventHandler.on(listener.event, listener.layer, listener.handler);
            }
            else {
    ...
    if (self._m) {
      Object.keys(listeners).forEach(function(event) {
        if (!drag[event]) {
          listeners[event].forEach(function (listener) {
            if (listener.layer) {
              self._featureEventHandler.off(listener.event, listener.layer, listener.handler);
            }
            else {
